<div style="border-bottom: 1px solid rgba(128, 128, 128, 0.4); padding: 5px; margin-bottom: -0.6em">
<h2 style="border-bottom: 0; margin-bottom: 0.1em;" align="center">Hi, I'm Mustafa 👋</h2>
<p style="opacity:0.8;"align="center">I Talk to Data and Generate Decisions.</p>
</div>
<br/>
<p>I hold a Master's of Data Analytics and a Bachelor in Computer Science. Passionate about using data to solve real-world problems and extracting insights. I utilize advanced analytical, statistical, and predictive methods to support data-driven decision-making.</p>

<h3>My Go-To Tools:</h3>
<ul class="items-grid">
  <li class="item" >Python</li>
  <li class="item" >SQL</li>
  <li class="item" >Pandas</li>
  <li class="item" >Numby</li>
  <li class="item" >Seaborn</li>
  <li class="item" >Plotly</li>
  <li class="item" >Excel</li>
  <li class="item" >PowerBI</li>
  <li class="item" >Tableau</li>
  <li class="item" >Tensorflow</li>
  <li class="item" >HuggingFace</li>
  <li class="item" >SK-Learn</li>
  <li class="item" >Apache Spark</li>
</ul>

<h3>Featured Projects</h3>
<div class="projects-container">
  <div class="project">
    <h4 class="title">Weather-Sensitive Demand Forecasting & Workforce Optimization: Ontario Retail Sector</h4>
    <ul class="items-grid s">
    <li class="s-li">Data Wrangling</li>
    <li class="s-li">Linear Programming</li>
    <li class="s-li">Machine Learning</li>
    </ul>
    <p>Addressed limitations of historical demand-based workforce planning by incorporating daily weather conditions into retail demand forecasting and staffing decisions. Integrated retail sales, weather, and labor-market data to evaluate weather-sensitive SARIMAX models and optimize workforce allocation using linear programming, reducing workforce mismatch by 12.5% and staffing penalty costs by 11.6% in the sector where weather significantly improved forecast accuracy.</p>
  </div>
    <div class="project">
    <h4 class="title">Weather-Sensitive Demand Forecasting & Workforce Optimization: Ontario Retail Sector</h4>
    <ul class="items-grid s">
    <li class="s-li">Data Wrangling</li>
    <li class="s-li">Linear Programming</li>
    <li class="s-li">Machine Learning</li>
    </ul>
        <p>Addressed limitations of historical demand-based workforce planning by incorporating daily weather conditions into retail demand forecasting and staffing decisions. Integrated retail sales, weather, and labor-market data to evaluate weather-sensitive SARIMAX models and optimize workforce allocation using linear programming, reducing workforce mismatch by 12.5% and staffing penalty costs by 11.6% in the sector where weather significantly improved forecast accuracy.</p>
  </div>
</div>

---

<div class="socials-container">
<a class="social" target="_blank" href="">
<img>
</a>
</div>

<style>
  .items-grid{
    font-size:14px;
    display:flex;
    gap:0.6em 0.4em;
    max-width:100%;
    flex-wrap:wrap;
    margin-bottom: 1em;
    list-style: none;
    padding-left: 0;
    list-style-position: inside;
    margin-bottom:0.7em;
  }
  .items-grid.s{
    gap:0.4em 0.2em;
  }
  li{
    padding: 0.2em 0.8em;
    border: 1px solid rgba(128, 128, 128, 0.4);
    border-radius: 99px;
    margin-bottom: 0em;
  }
  li.s-li{
    font-size: 12px;
    padding: 0em 0.6em;
  }
  .projects-container{
    display: flex;
    gap: 1em;
  }
  .project{
    border: 1px solid rgba(128, 128, 128, 0.4);
    border-radius: 20px;
    padding: 1em 1.2em
  }
  .project .title{
    padding-bottom: 0.6em;
    border-bottom: solid 1px rgba(128, 128, 128, 0.4);
    margin: 0 0 0.6em 0;
  }

</style>
